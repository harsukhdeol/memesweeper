...
void MemeField::Tile::Draw( const Vei2& screenPos, MemeField::State gameState,Graphics& gfx ) const
{
	if(gameState == MemeField::State::memeing)
	{
		switch( state )
		{
		case State::Hidden:
...
bool MemeField::Tile::HasNoNeighboringMemes() const
{
...
{
	gfx.DrawRect(GetRect().GetExpanded(thickness), c);
...
void MemeField::RevealTile(const Vei2& gridPos)
{
	...
	if (!tile.IsRevealed() && !tile.IsFlagged())
	{
		...
		else if (tile.HasNoNeighboringMemes())
		{
			for (Vei2 gridPos = { xStart,yStart }; gridPos.y <= yEnd; gridPos.y++)
			{
				for (gridPos.x = xStart; gridPos.x <= xEnd; gridPos.x++)
